# Apache Spark + JDK: Полный гайд по установке (Windows & macOS)

Этот туториал показывает, как с нуля установить **Apache Spark** и **JDK** так, чтобы:

- Spark работал из терминала/командной строки;
- можно было проверять версии и запускать простые примеры;
- студенты могли повторить шаг за шагом.

## 0. Рекомендуемые версии

**Рекомендованный стек (и для Windows, и для macOS):**

| Компонент    | Версия                               |
|--------------|--------------------------------------|
| Java (JDK)   | **JDK 17 (LTS)**                     |
| Apache Spark | **Spark 3.5.7 (`spark-3.5.7-bin-hadoop3`)** |

Spark 3.5.x официально поддерживает Java 8/11/17, поэтому JDK 17 — безопасный выбор на несколько лет вперёд.

---

## Часть A. Установка Apache Spark 3.5.7 на Windows 10/11

### A1. Устанавливаем JDK 17 (Windows)

1. Скачайте JDK 17 (любую OpenJDK-сборку), например:
   - Adoptium Temurin 17 (MSI для Windows x64)
   - или Microsoft OpenJDK 17
2. Установите JDK (Next–Next–Finish).
   Типовой путь:
   - `C:\Program Files\Eclipse Adoptium\jdk-17.x.x.x-hotspot`
   - или `C:\Program Files\Java\jdk-17.x.x`

3. Проверьте установку в **cmd**:

   ```cmd
   java -version

openjdk version "17.0.11" 2024-04-16
OpenJDK Runtime Environment ...
OpenJDK 64-Bit Server VM ...
Если команда не найдена — значит ещё не настроен PATH / JAVA_HOME.

A2. Переменные среды для Java (Windows)
Откройте:

Win + X → System (или «Параметры → Система → О системе»),

справа/снизу — Дополнительные параметры системы,

в окне «Свойства системы» нажмите кнопку Переменные среды....

В блоке Системные переменные (нижняя часть окна) нажмите Создать...:

Имя переменной: JAVA_HOME

Значение переменной: путь к установленному JDK, например:
C:\Program Files\Eclipse Adoptium\jdk-17.0.11.9-hotspot

В списке Системные переменные найдите Path → Изменить... → Создать:

Добавьте строку:

text
Копировать код
%JAVA_HOME%\bin
Нажмите OK во всех окнах, закройте все открытые cmd / PowerShell, затем откройте новое окно cmd.

Проверьте:

cmd
Копировать код
java -version
echo %JAVA_HOME%
A3. Скачивание и распаковка Apache Spark 3.5.7 (Windows)
Зайдите на официальный сайт Apache Spark и скачайте:

Release: 3.5.7

Package type: Pre-built for Apache Hadoop 3

Архив: spark-3.5.7-bin-hadoop3.tgz

Создайте папку для инструментов через cmd:

cmd
Копировать код
C:
cd \
mkdir tools
cd tools
mkdir spark
В итоге будет папка: C:\tools\spark.

Распакуйте архив (spark-3.5.7-bin-hadoop3.tgz) при помощи 7-Zip:

Правый клик → 7-Zip → Extract Here → появится spark-3.5.7-bin-hadoop3.tar

Ещё раз правый клик на .tar → **7-Zip → Extract to "spark-3.5.7-bin-hadoop3"`

Переместите папку spark-3.5.7-bin-hadoop3 в C:\tools\spark.

Итоговый путь к Spark:

text
Копировать код
C:\tools\spark\spark-3.5.7-bin-hadoop3
A4. Установка winutils.exe (Hadoop tools для Windows)
На Windows Spark использует некоторые функции Hadoop, которым нужен winutils.exe.

В cmd создайте папки:

cmd
Копировать код
C:
cd \
mkdir hadoop
cd hadoop
mkdir bin
Теперь есть C:\hadoop\bin.

Скачайте winutils.exe для Hadoop 3.3.x (пример через curl):

cmd
Копировать код
curl --ssl-no-revoke -L -o C:\hadoop\bin\winutils.exe ^
  https://github.com/cdarlint/winutils/raw/master/hadoop-3.3.5/bin/winutils.exe
⚠️ Это не официальный бинарь Apache, а удобный костыль для локальной разработки на Windows.

Проверьте, что файл скачался:

cmd
Копировать код
cd C:\hadoop\bin
dir
В списке файлов должен быть winutils.exe.

A5. Настройка SPARK_HOME и HADOOP_HOME (Windows)
Снова откройте окно «Переменные среды» (как в шаге A2).

В блоке Системные переменные нажмите Создать... и добавьте:

Переменная 1:

Имя: SPARK_HOME

Значение: C:\tools\spark\spark-3.5.7-bin-hadoop3

Переменная 2:

Имя: HADOOP_HOME

Значение: C:\hadoop

В списке системных переменных найдите Path → Изменить... → Создать и добавьте строки:

text
Копировать код
%SPARK_HOME%\bin
%HADOOP_HOME%\bin
Нажмите OK во всех окнах, закройте cmd, откройте новое окно cmd.

Проверьте:

cmd
Копировать код
echo %SPARK_HOME%
echo %HADOOP_HOME%
spark-shell --version
При успехе вы увидите версию Spark (3.5.7) и он запустит Scala shell.

A6. Быстрая проверка Spark на Windows
Scala shell
cmd
Копировать код
spark-shell
После загрузки увидите приглашение:

text
Копировать код
scala>
Можно выполнить тест:

scala
Копировать код
scala> sc.parallelize(1 to 5).map(_ * 2).collect()
res0: Array[Int] = Array(2, 4, 6, 8, 10)
PySpark (интерактивный режим)
cmd
Копировать код
pyspark
В конце:

text
Копировать код
Python 3.x.x (default, ...)
Type "help", "copyright", ...
>>>
Проверка контекста:

python
Копировать код
>>> sc
>>> sc.version
Версия должна быть 3.5.7.

A7. Тестовый скрипт PySpark (Windows)
Создайте файл C:\tools\spark\test_spark.py:

python
Копировать код
from pyspark.sql import SparkSession

spark = SparkSession.builder \
    .appName("TestSpark") \
    .getOrCreate()

print("Spark version:", spark.version)

df = spark.createDataFrame([(1, "Alice"), (2, "Bob")], ["id", "name"])
df.show()

spark.stop()
Запустите:

cmd
Копировать код
cd C:\tools\spark
python test_spark.py
Ожидаемый результат: в консоли появится версия Spark и табличка с двумя строками (Alice и Bob).

Часть B. Установка Apache Spark 3.5.7 на macOS (Intel / Apple Silicon)
B1. Установка JDK 17 через Homebrew
Откройте Terminal и проверьте Homebrew:

bash
Копировать код
brew --version
Если команда не найдена — установите Homebrew по инструкции с официального сайта.

Установите JDK 17:

bash
Копировать код
brew install openjdk@17
Добавьте JDK 17 в PATH и задайте JAVA_HOME (для zsh):

bash
Копировать код
echo 'export PATH="/opt/homebrew/opt/openjdk@17/bin:$PATH"' >> ~/.zshrc
echo 'export JAVA_HOME=$(/usr/libexec/java_home -v 17)' >> ~/.zshrc
source ~/.zshrc
На Intel Mac путь может быть /usr/local/opt/openjdk@17/..., но команда
/usr/libexec/java_home -v 17 сама подставит правильный каталог.

Проверьте:

bash
Копировать код
java -version
echo $JAVA_HOME
B2. Скачивание и распаковка Spark 3.5.7 (macOS)
Создайте папку под инструменты и скачайте архив:

bash
Копировать код
mkdir -p ~/tools
cd ~/tools
curl -L -O https://dlcdn.apache.org/spark/spark-3.5.7/spark-3.5.7-bin-hadoop3.tgz
Распакуйте архив:

bash
Копировать код
tar -xvf spark-3.5.7-bin-hadoop3.tgz
Папка с Spark:

text
Копировать код
~/tools/spark-3.5.7-bin-hadoop3
B3. Настройка SPARK_HOME и PATH на macOS
Откройте / создайте файл ~/.zshrc и добавьте строки:

bash
Копировать код
export SPARK_HOME="$HOME/tools/spark-3.5.7-bin-hadoop3"
export PATH="$SPARK_HOME/bin:$PATH"
Примените настройки:

bash
Копировать код
source ~/.zshrc
Проверьте:

bash
Копировать код
echo $SPARK_HOME
spark-shell --version
Должна выводиться версия Spark 3.5.7.

💡 На macOS НЕ нужен winutils.exe — это костыль исключительно для Windows.

B4. Проверка Spark на macOS
Scala shell
bash
Копировать код
spark-shell
В консоли:

scala
Копировать код
scala> sc.parallelize(1 to 5).sum()
res0: Double = 15.0
PySpark
bash
Копировать код
pyspark
Внутри REPL:

python
Копировать код
>>> sc
>>> sc.version
B5. Тестовый скрипт PySpark (macOS)
Создайте файл ~/tools/test_spark.py:

python
Копировать код
from pyspark.sql import SparkSession

spark = SparkSession.builder \
    .appName("TestSparkMac") \
    .getOrCreate()

print("Spark version:", spark.version)

df = spark.range(5)
df.show()

spark.stop()
Запуск:

bash
Копировать код
cd ~/tools
python3 test_spark.py
При успехе будет выведена версия Spark и таблица с числами 0–4.

Часть C. PySpark в Jupyter Notebook
Вариант 1. PySpark через pip (минимум боли для студентов)
В той же среде (виртуальное окружение / Conda), где установлен Jupyter, выполните:

bash
Копировать код
pip install pyspark
Запустите Jupyter (Notebook / Lab) и в новой ячейке:

python
Копировать код
from pyspark.sql import SparkSession

spark = SparkSession.builder \
    .appName("JupyterTest") \
    .getOrCreate()

print("Spark:", spark.version)
spark.range(10).show()
PySpark подтянет совместимый Spark (если не задан SPARK_HOME) или будет использовать уже настроенный Spark.

Вариант 2. Использовать установленный Spark (через SPARK_HOME)
Для обоих вариантов (Windows и macOS) важно, чтобы были настроены:

JAVA_HOME → путь к установленному JDK 17;

SPARK_HOME → путь к директории Spark spark-3.5.7-bin-hadoop3;

PATH содержит $SPARK_HOME/bin (или %SPARK_HOME%\bin на Windows).

В этом случае, когда вы установите PySpark (pip install pyspark) и вызовете его в ноутбуке, он сможет использовать уже установленный Spark.

Часть D. Мини-чеклист для отладки (Windows & macOS)
Если Spark не запускается или не создаётся SparkContext:

java -version
Убедитесь, что используется Java 17, а не 1.8 / 11 / 21.

JAVA_HOME

Windows: echo %JAVA_HOME%

macOS: echo $JAVA_HOME
Путь должен указывать на директорию JDK 17.

SPARK_HOME

Windows: echo %SPARK_HOME%

macOS: echo $SPARK_HOME
Должен указывать на директорию spark-3.5.7-bin-hadoop3.

PATH
В PATH должны фигурировать:

Windows: %JAVA_HOME%\bin, %SPARK_HOME%\bin, %HADOOP_HOME%\bin (если на Windows).

macOS: $JAVA_HOME/bin, $SPARK_HOME/bin.

Проверка spark-shell --version
Если shell вообще не стартует — ошибка в JAVA_HOME или SPARK_HOME.

Для Windows:

Проверьте наличие C:\hadoop\bin\winutils.exe.

Проверьте переменную HADOOP_HOME и наличие %HADOOP_HOME%\bin в Path.

Для Jupyter:

Установлен ли пакет pyspark в той же среде, где запущен Jupyter (pip show pyspark)?

Не конфликтуют ли несколько версий Python / разные окружения?

Этот README можно использовать:

как учебный шаблон для студентов (шаги 0 → A → B → C → D),

как чеклист для настройки окружения перед курсом по Spark / Data Engineering / Big Data,

как основу для дальнейших расширений (Docker, Kubernetes, Spark-кластер и т.д.).
